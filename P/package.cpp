...
std::vector<ElementID> Package::freed_IDs_{0};
std::vector<ElementID> Package::assigned_IDs_{};

Package::Package() {
    if(!freed_IDs_.empty()){
        auto p2newID = std::min(freed_IDs_.begin(), freed_IDs_.end());
        elementId_ = *p2newID;
        freed_IDs_.erase(p2newID);
        assigned_IDs_.push_back(*p2newID);
    }else{
        ElementID newID = (*std::max(freed_IDs_.begin(), freed_IDs_.end())) + 1;
        elementId_ = newID;
...
Package::Package(ElementID id2assign) {
    if(std::find(assigned_IDs_.begin(), assigned_IDs_.end(), id2assign) == assigned_IDs_.end()){
        ...
        elementId_ = (*std::max(freed_IDs_.begin(), freed_IDs_.end())) + 1;
    }
};
